...
export async function updateUsuario(req: Request, res: Response) {
  Usuario.findByIdAndUpdate({ _id: req.params.id }, req.body)
    .then(() => {
      res.json("Actualizacion de Usuario realizada");
    })
    .catch(err => {
...
export async function login(req: Request, res: Response) {
  ...
  const correctPass: boolean = await usuarioEncontrado.validatePassword(
    user.password
  );
